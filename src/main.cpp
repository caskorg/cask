...
boost::property_tree::ptree write_params(const cask::runtime::ImplementationParameters& params) {
  boost::property_tree::ptree tree;
  tree.put("num_pipes", params.num_pipes);
  tree.put("cache_size", params.cache_size);
  tree.put("input_width", params.input_width);
  tree.put("max_rows", params.max_rows);
  tree.put("num_controllers", params.num_controllers);
  return tree;
}
...
void write_dse_results(
  ...
  for (const auto& dseResult : results) {
    ...
    archJson.add_child("architecture_params", write_params(arch->impl.params));
